...
import {
  Download, Loader2, RefreshCw, ChevronRight, ArrowUpRight, ArrowDownRight,
...
function SelfieModal({ dispatch, type, onClose }) {
  ...
  const [photo,setPhoto]=useState(null),[url,setUrl]=useState(null)
  const [uploading,setUploading]=useState(false),[loc,setLoc]=useState({})
  useEffect(() => {
    ...
    <Modal isOpen={true} onClose={onClose} title={type==='checkin'?'Check In':'Check Out'} size="md">
      <div className="space-y-4">
        ...
        <div className="flex gap-3">
          {photo?(<><button onClick={()=>{setPhoto(null);setUrl(null);navigator.mediaDevices?.getUserMedia({video:{facingMode:'user'}}).then(s=>{sRef.current=s;if(vRef.current)vRef.current.srcObject=s})}} className="flex-1 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 text-sm text-gray-600">↩ Retake</button><Button onClick={confirm} disabled={!url||uploading} loading={loading.action} className="flex-1"><CheckCircle2 size={15}/> Confirm</Button></>):(<Button onClick={capture} className="flex-1">📸 Take Selfie</Button>)}
        </div>
      </div>
    </Modal>
  )
...
// ─── SECTION 1: Stat Cards (5 cards in a row) ─────────────────────────────────
function StatCards({ stats, loading, onAttendance }) {
  const navigate = useNavigate()
  const s = stats?.stats
  const cards = [
    {
      label:'Total Leads', value: s?.total_leads?.value??0,
      sub:`${s?.total_leads?.booked??0} booked`,
      badge: s?.total_leads?.conversion_rate ? `${s.total_leads.conversion_rate}% conv.` : null,
    ...
    {
      label:'Site Visits', value: s?.total_site_visits?.value??0,
      sub:`${s?.total_site_visits?.upcoming??0} upcoming`,
      badge:`${s?.total_site_visits?.done??0} done`,
    ...
    {
      label:'Follow-Ups', value: s?.total_follow_ups?.value??0,
      sub:`${s?.total_follow_ups?.pending??0} pending`,
      badge: s?.total_follow_ups?.overdue>0 ? `${s.total_follow_ups.overdue} overdue` : null,
      badgeColor:'text-red-600 bg-red-50 dark:bg-red-900/20',
      icon:Phone, grad:'from-green-500 to-teal-500',
      ring:'hover:ring-green-100 dark:hover:ring-green-900', path:'/follow-ups',
    },
    {
      label:'Projects', value: s?.total_projects?.value??0,
      sub:`${s?.total_projects?.active??0} active`,
      badge:`${s?.total_projects?.upcoming??0} upcoming`,
      badgeColor:'text-amber-600 bg-amber-50 dark:bg-amber-900/20',
      icon:Building2, grad:'from-amber-400 to-orange-500',
      ring:'hover:ring-amber-100 dark:hover:ring-amber-900', path:'/projects',
    },
    {
  ...
  if (loading) return (
    <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
      {[1,2,3,4,5].map(i=>(
        <div key={i} className="bg-white dark:bg-[#1a1a1a] rounded-2xl p-4 border border-gray-100 dark:border-gray-800 animate-pulse h-[108px]"/>
      ))}
    </div>
  )
      ...
      {cards.map((c,i)=>(
        <div key={i} onClick={c.onClick||(()=>navigate(c.path))}
          ...
          <div className="flex items-start justify-between mb-2.5">
            <div className={`w-8 h-8 rounded-xl bg-gradient-to-br ${c.grad} flex items-center justify-center shadow-sm`}>
...
// ─── SECTION 2: Quick Actions + Upcoming Visits + Recent Activity ─────────────
function QuickActionsRow({ navigate }) {
  const actions = [
    { label:'Leads',         icon:Users,         color:'bg-blue-500',   grad:'from-blue-400 to-blue-600',     path:'/leads'         },
    { label:'Site Visits',   icon:CalendarDays,  color:'bg-purple-500', grad:'from-purple-400 to-violet-600', path:'/site-visits'   },
    { label:'Follow-Ups',    icon:Phone,         color:'bg-green-500',  grad:'from-green-400 to-teal-500',    path:'/follow-ups'    },
    { label:'Projects',      icon:Building2,     color:'bg-amber-500',  grad:'from-amber-400 to-orange-500',  path:'/projects'      },
    { label:'Team',          icon:UserCheck,     color:'bg-pink-500',   grad:'from-pink-400 to-rose-500',     path:'/team'          },
    { label:'Attendance',    icon:ClipboardList, color:'bg-rose-500',   grad:'from-rose-400 to-red-500',      path:'/attendance'    },
    { label:'Notifications', icon:Bell,          color:'bg-teal-500',   grad:'from-teal-400 to-cyan-500',     path:'/notifications' },
    { label:'Reports',       icon:BarChart3,     color:'bg-indigo-500', grad:'from-indigo-400 to-blue-600',   path:'/attendance'    },
  ]
  return (
      ...
      <div className="grid grid-cols-2 gap-2 flex-1">
        {actions.map((a,i)=>(
          <button key={i} onClick={()=>navigate(a.path)}
            ...
            <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${a.grad} flex items-center justify-center shadow-sm flex-shrink-0 group-hover:scale-105 transition-transform duration-150`}>
...
function UpcomingVisitsCard({ navigate }) {
  ...
  return (
    <div className="bg-white dark:bg-[#1a1a1a] rounded-2xl border border-gray-100 dark:border-gray-800 p-4 h-full flex flex-col">
      ...
      {loading.siteVisits?(
        <div className="space-y-2">{[1,2,3].map(i=><div key={i} className="h-11 bg-gray-100 dark:bg-gray-800 rounded-xl animate-pulse"/>)}</div>
      ):upcomingSiteVisits.length===0?(
        <div className="flex-1 flex flex-col items-center justify-center gap-2 py-4">
          <MapPin size={20} className="text-gray-200 dark:text-gray-700"/>
          ...
          {upcomingSiteVisits.slice(0,5).map((v,i)=>(
            <div key={i} className="flex items-center gap-2.5 p-2.5 rounded-xl bg-gray-50 dark:bg-gray-800/40 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
              <div className="flex-shrink-0">
                <div className={`w-2 h-2 rounded-full ${STATUS_DOT[v.status]||'bg-gray-400'}`}/>
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold text-gray-800 dark:text-gray-200 truncate">{v.lead_name}</p>
...
function RecentActivityCard({ navigate }) {
  ...
  return (
    <div className="bg-white dark:bg-[#1a1a1a] rounded-2xl border border-gray-100 dark:border-gray-800 p-4 h-full flex flex-col">
      ...
      {loading.activity?(
        <div className="space-y-2">{[1,2,3,4].map(i=><div key={i} className="h-10 bg-gray-100 dark:bg-gray-800 rounded-xl animate-pulse"/>)}</div>
      ):recentActivity.length===0?(
        <div className="flex-1 flex flex-col items-center justify-center gap-2 py-4">
          <Activity size={20} className="text-gray-200 dark:text-gray-700"/>
...
// ─── SECTION 3: Lead Pipeline + Lead Sources ──────────────────────────────────
  ...
  return (
    <div className="bg-white dark:bg-[#1a1a1a] rounded-2xl border border-gray-100 dark:border-gray-800 p-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="text-sm font-semibold text-gray-800 dark:text-gray-200">Lead Pipeline</p>
          <p className="text-[11px] text-gray-400 mt-0.5">Current distribution across stages</p>
        </div>
        <div className="w-8 h-8 rounded-xl bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center">
          <Target size={15} className="text-[#0082f3]"/>
        </div>
      </div>
      {loading.pipeline?(
        <div className="space-y-3">{[1,2,3,4,5,6].map(i=><div key={i} className="h-8 bg-gray-100 dark:bg-gray-800 rounded-xl animate-pulse"/>)}</div>
      ):stages.length===0?(
        <div className="flex flex-col items-center justify-center py-8 gap-2">
          <Target size={28} className="text-gray-200 dark:text-gray-700"/>
            ...
            return(
              <div key={i} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                  <div className="h-full rounded-full transition-all duration-700" style={{width:`${pct||0}%`,backgroundColor:cfg.color}}/>
                </div>
...
function LeadSourcesCard() {
  ...
  const CustomTooltip=({active,payload})=>{
    if(active&&payload?.length){
      ...
      return(
        <div className="bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2 shadow-xl text-xs">
          <p className="font-semibold text-gray-800 dark:text-gray-200">{d.source}</p>
          <p className="text-gray-500">{d.count} leads · {d.percentage}%</p>
        </div>
      )
    }
    return null
  ...
  return (
    <div className="bg-white dark:bg-[#1a1a1a] rounded-2xl border border-gray-100 dark:border-gray-800 p-4">
      <div className="flex items-center justify-between mb-3">
        <div>
          <p className="text-sm font-semibold text-gray-800 dark:text-gray-200">Lead Sources</p>
          <p className="text-[11px] text-gray-400 mt-0.5">{total>0?`${total} total leads · distribution`:' Distribution by source'}</p>
        </div>
      ...
      {loading.leadSources?(
        <div className="h-44 flex items-center justify-center"><Loader2 size={20} className="animate-spin text-[#0082f3]"/></div>
      ):chart.length===0?(
        <div className="flex flex-col items-center justify-center py-6 gap-2">
          <Zap size={28} className="text-gray-200 dark:text-gray-700"/>
          <p className="text-xs text-gray-400">No source data yet — add leads to see distribution</p>
        </div>
      ):(
        <div className="flex items-center gap-4">
          <div className="w-36 h-36 flex-shrink-0">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={chart} cx="50%" cy="50%" innerRadius={36} outerRadius={58} paddingAngle={2} dataKey="count" nameKey="source">
                  {chart.map((e,i)=><Cell key={i} fill={e.color||'#cbd5e1'} stroke="none"/>)}
                </Pie>
      ...
      {/* Show all sources legend below if needed */}
      {all.filter(s=>s.count===0).length>0&&(
        <div className="grid grid-cols-5 gap-1 mt-3 pt-3 border-t border-gray-50 dark:border-gray-800">
          {all.filter(s=>s.count===0).map((s,i)=>(
            <div key={i} className="flex items-center gap-1">
              <div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{backgroundColor:s.color||'#cbd5e1',opacity:0.3}}/>
              <span className="text-[9px] text-gray-300 dark:text-gray-700 truncate">{s.source}</span>
            </div>
          ))}
        </div>
      )}
...
// ─── SECTION 4: Revenue Trend + Commission ────────────────────────────────────
function RevenueCard() {
  ...
  return (
    <div className="bg-white dark:bg-[#1a1a1a] rounded-2xl border border-gray-100 dark:border-gray-800 p-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="text-sm font-semibold text-gray-800 dark:text-gray-200">Lead Trend</p>
          <p className="text-[11px] text-gray-400 mt-0.5">{tf==='Week'?'Daily (last 7 days)':tf==='Year'?'Monthly this year':'Daily this month'}</p>
        </div>
        <div className="flex bg-gray-100 dark:bg-gray-800 rounded-lg p-0.5 gap-0.5">
          {['Week','Month','Year'].map(t=>(
            <button key={t} onClick={()=>setTf(t)} className={`px-2.5 py-1 rounded-md text-[11px] font-medium transition-all ${tf===t?'bg-white dark:bg-gray-700 text-gray-900 dark:text-white shadow-sm':'text-gray-500 dark:text-gray-400'}`}>{t}</button>
          ))}
        </div>
      </div>
      <div className="h-44">
        {loading.revenue?(
          <div className="h-full flex items-center justify-center"><Loader2 size={18} className="animate-spin text-[#0082f3]"/></div>
        ):data.length===0?(
          <div className="h-full flex flex-col items-center justify-center gap-2">
            <TrendingUp size={28} className="text-gray-200 dark:text-gray-700"/>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} margin={{top:2,right:4,left:-24,bottom:0}} barCategoryGap="30%">
              ...
              <Tooltip
                contentStyle={{backgroundColor:'#fff',border:'1px solid #f1f5f9',borderRadius:'10px',fontSize:'11px',boxShadow:'0 4px 24px rgba(0,0,0,0.08)'}}
                cursor={{fill:'rgba(0,130,243,0.04)'}}
              />
    ...
    <div className="bg-white dark:bg-[#1a1a1a] rounded-2xl border border-gray-100 dark:border-gray-800 p-4 flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-[11px] text-gray-400 mt-0.5">Key metrics snapshot</p>
        </div>
...
export default function Dashboard() {
  ...
  const handleExportSubmit = async (dateRange) => {
    try {
      ...
      a.href = url
      a.download = `${exportModule.replace('-', '_')}_${dateRange.from}_to_${dateRange.to}.xlsx`
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        ...
        <ExportMenu 
          ...
          setExportModule={setExportModule} 
          exportingKey={exportingKey} 
        />
      </div>

      {/* ── Row 1: 5 Stat Cards ─────────────────────────────────────────── */}
      <StatCards stats={stats} loading={loading.stats} onAttendance={()=>setShowAtt(true)}/>

      {/* ── Row 2: Quick Access (1/5 = 20%) + Upcoming Visits (2/5 = 40%) + Activity (2/5 = 40%) */}
      <div className="grid grid-cols-1 lg:grid-cols-6 gap-3">
        <div className="lg:col-span-2"><QuickActionsRow navigate={navigate}/></div>
        <div className="lg:col-span-2"><UpcomingVisitsCard navigate={navigate}/></div>
        <div className="lg:col-span-2"><RecentActivityCard navigate={navigate}/></div>
      </div>

      {/* ── Row 3: Pipeline (1/2) + Sources (1/2) ───────────────────────── */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <PipelineCard/>
        <LeadSourcesCard/>
      </div>

      {/* ── Row 4: Revenue (2/3) + Performance (1/3) ────────────────────── */}
