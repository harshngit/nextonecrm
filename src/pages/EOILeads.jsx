  ...
  if (canSeePhone) {
    return (
      <div className="flex flex-col gap-0.5">
        <PhoneActions phone={lead.phone} email={lead.email} projectId={lead.project_id} projectName={lead.project_name}>
...
export default function EOILeads() {
  ...
  // leaving this page (e.g. to view a lead) and coming back via Back
  // restores the exact page AND page-size the user was on.
  useEffect(() => {
  ...
  // Status isn't included — this page is always scoped to 'eoi'.
  const [exportFilters, setExportFilters] = useState({
    source: '', assignedTo: '', projectId: '', projectName: '', location: '', search: '',
  })
  ...
  // status change, reassign, delete, ...). Building these params in two
  // separate places previously let the post-action refetch silently drop the
  // fixed status="eoi" filter this page always applies, so a lead whose
  // status had just been changed away from it would still show up here until
  // the next filter-driven refetch.
  const fetchAllLeads = () => {
  ...
  const openExportModal = () => {
    setExportFilters({
      source: filterSource, assignedTo: filterAssigned,
  ...
  const handleExport = async (dateRange) => {
    try {
      ...
      if (filterStatus) params.status = filterStatus // always 'eoi' on this page
      ...
      const a = document.createElement('a'); a.href = url; a.download = `EOILeads_${dateRange.from}_to_${dateRange.to}.xlsx`
            ...
            <table className="w-full min-w-[780px] text-sm">
              <thead>
                <tr className="border-b border-gray-200 dark:border-gray-800 bg-blue-50/50 dark:bg-blue-900/10">
                  ...
                  {['Lead', 'Phone', 'Source', 'Assigned', 'Status', 'Project', 'Created', 'Actions']
